...
function bootpopup(options) {
	...
	this.create = function() {
		...
		this.modal = $('<div id="confirmModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="bootpopup-title"></div>');
		...
		if(this.options.showclose)	// Close button
			this.header.append('<button type="button" class="bootpopup-button close" data-dismiss="modal" aria-label="Fermer"><img src="/images/CROIX_FERMETURE_BLANC.svg" height="32px" width="32px" /></button>');
		...
		// Footer
		this.footer = $('<div class="modal-footer"><div class="confirm-buttons"></div></div>');
		...
		for(key in this.options.buttons) {
			...
			switch(item) {
				case "close": btnClass = "button"; btnText = "Fermer"; spanClass = "btn_close"; break;
				case "ok": btnClass = "button button_bold"; btnText = "OK"; spanClass = "btn_save"; break;
				case "cancel": btnClass = "button"; btnText = "Annuler"; spanClass = "btn_cancel"; break;
				case "yes": btnClass = "button button_bold"; btnText = "Oui"; spanClass = "btn_save"; break;
				case "no": btnClass = "button"; btnText = "Non"; spanClass = "btn_delete"; break;
			}

			var button = $('<div class="' + btnClass + '"><a data-dismiss="modal" data-callback="' + item + '" data-form="' + this.formid + '">' + btnText + '</a><span class="' + spanClass + '"></span></div>').on('click', function(event) {
				var name = $(event.target).data("callback");
				self.callback(name, event);
			});
			this.footer.find('div.confirm-buttons').append(button);
