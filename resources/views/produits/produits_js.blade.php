	...
	*/
	...
	function loadDatatableCentrals(rowData, json)
	{
		var data = jQuery.map(json, function(el, i) {
			return [[el.centrale_nom, el.code_produit, el.denomination, el.prix_unitaire, el.cp_id, el.obsolete, el.date_obsolescence, el.date_denomination, el.date_prix_unitaire]];
		});
		...
		tableCentrals = $('#tab-product-centrals-' + idRow).DataTable( {
			...
			"aoColumns": [ 
				...
				{ "render": function ( data, type, row ) { 
					return (data != null) ? data + ((row[6] != null) ? ' (' + row[6] + ')' : '') : '-'; 
				}} , 
				{ "render": function ( data, type, row ) { 
					return (data != null) ? data + ((row[7] != null && row[7] != firstDay) ? ' <i>(' + row[7] + ')</i>' : '') : '-'; 
				}} , 
				{ "render": function ( data, type, row ) { 
					var dataResult = (data != null) ? (numberWithSpaces(parseFloat(data.replace( /,/, "." ).replace( / /g, "" )).toFixed(2))) : '-';

					return (data != null) ? (dataResult + ' €'+ ((row[8] != null && row[8] != firstDay) ? ' <i>(' + row[8] + ')</i>' : '')) : dataResult;
				}}
			],
			"aaData": data,
			"createdRow": function ( row, data, index ) {
				...
				$('td', row).eq(2).find('div').attr('title', $('td', row).eq(2).find('div').html().replace( '<i>', "" ).replace( '</i>', "" ));
				...
				if (data[5])
				{
					...
					if (data[6] != null)
						$('td', row).eq(1).find('div').attr('title', data[6]);
			...
			initComplete: function () {
				...
				{
					$('#mask-obsolete-' + idRow).prop( "checked", rowData[9] ).trigger("change");
				}
			}
		});
	...
	function loadAjaxFormProduits()
	{
		$.ajax({
		    ...
		    success: function(json) {
		    	var data = jQuery.map(json, function(el, i) {
				  return [[null, el.laboratoire, el.denomination, el.conditionnement, el.code_gtin, el.valo_euro, el.valo_volume, el.id, el.classe_therapeutique, el.obsolete, el.unite_valo_volume, el.types_id, el.especes_id, el.invisible]];
				});
			    ...
			    var table = $('#tab-produits').DataTable( {
					...
					"aoColumns": [ 
		            	...
		            	{ "render": function ( data, type, row ) {
			                  return (data != null) ? (numberWithSpaces(parseFloat(data.replace( /,/, "." ).replace( / /g, "" )).toFixed(2)) + (row[10] != null ? ' ' + row[10] : '')) : '-';
			            	}, 
			            	"sType": "numeric-comma"
			            }
			        ],
					...
					"createdRow": function ( row, data, index ) {
						...
						$('td', row).eq(5).find('div').css('padding-left', '5px');
						$('td', row).eq(5).find('div').css('padding-right', '5px');
			    ...
			    $('#tab-produits tbody').on('click', 'td div[class^="details-control"]', function () {
					...
					var isObsolete = row.data()[9] || row.data()[13];
